...
const HOTSPOTS: Hotspot[] = [
  { name: 'London',      desc: 'Fabric · Egg · Printworks',         lat: 51.5,  lon: -0.1,   tier: 2 },
  { name: 'Berlin',      desc: 'Berghain · Tresor · Watergate',     lat: 52.5,  lon: 13.4,   tier: 2 },
  { name: 'Ibiza',       desc: 'DC10 · Pacha · Amnesia',            lat: 38.9,  lon: 1.4,    tier: 2 },
  { name: 'Amsterdam',   desc: 'Shelter · Awakenings · ADE',        lat: 52.4,  lon: 4.9,    tier: 1 },
  { name: 'Paris',       desc: 'Rex Club · Concrete · Batofar',     lat: 48.9,  lon: 2.4,    tier: 1 },
  { name: 'Bristol',     desc: 'Motion · Marble Factory · Thekla',  lat: 51.5,  lon: -2.6,   tier: 1 },
  { name: 'Barcelona',   desc: 'Razzmatazz · Input · Apolo',        lat: 41.4,  lon: 2.2,    tier: 1 },
  { name: 'Detroit',     desc: 'Movement Festival · CODA',          lat: 42.3,  lon: -83.0,  tier: 1 },
  { name: 'Chicago',     desc: 'Smart Bar · Spy Bar',               lat: 41.9,  lon: -87.6,  tier: 1 },
  { name: 'New York',    desc: 'Output · House of Yes · Nowadays',  lat: 40.7,  lon: -74.0,  tier: 1 },
  { name: 'Rio',         desc: 'Fosfobox · Green Valley',           lat: -22.9, lon: -43.2,  tier: 1 },
  { name: 'Sao Paulo',   desc: 'D-Edge · Lov · The Week',           lat: -23.6, lon: -46.6,  tier: 1 },
  { name: 'Bangkok',     desc: 'De Commune · Onyx · Beam',          lat: 13.8,  lon: 100.5,  tier: 1 },
  { name: 'Koh Phangan', desc: 'Full Moon Party · Shiva Moon',      lat: 9.7,   lon: 100.0,  tier: 1 },
  { name: 'Melbourne',   desc: 'Revolver · Brown Alley · 161',      lat: -37.8, lon: 145.0,  tier: 1 },
  { name: 'Tokyo',       desc: 'Womb · Contact · Dommune',          lat: 35.7,  lon: 139.7,  tier: 1 },
  { name: 'Bali',        desc: 'Potato Head · La Favela',           lat: -8.4,  lon: 115.2,  tier: 0 },
  { name: 'Cape Town',   desc: 'Shimmy Beach · Cause Effect',       lat: -33.9, lon: 18.4,   tier: 0 },
  { name: 'Prague',      desc: 'Ankali · Cross Club',               lat: 50.1,  lon: 14.4,   tier: 0 },
  { name: 'Zurich',      desc: 'Hive · Zukunft · Supermarket',      lat: 47.4,  lon: 8.5,    tier: 0 },
  { name: 'Dubai',       desc: 'White Dubai · Soho Garden',         lat: 25.2,  lon: 55.3,   tier: 0 },
  { name: 'Seoul',       desc: 'Cakeshop · Contra · Boogie Nights', lat: 37.6,  lon: 127.0,  tier: 0 },
  { name: 'LA',          desc: 'Exchange · Catch One · EDC',        lat: 34.1,  lon: -118.2, tier: 0 },
  { name: 'Toronto',     desc: 'Coda · Rebel · Velvet Underground', lat: 43.7,  lon: -79.4,  tier: 0 },
  { name: 'Bogota',      desc: 'Baum · Video Club · Armando',       lat: 4.7,   lon: -74.1,  tier: 0 },
]

const GLOW_R: Record<0|1|2,number> = { 0: 4, 1: 6, 2: 9 }
function hotspotGlow(lat: number, lon: number): number {
  let max = 0
  for (const h of HOTSPOTS) {
    const d = Math.sqrt((lat-h.lat)**2+(lon-h.lon)**2)
    const r = GLOW_R[h.tier]
    if (d < r) { const g=(1-d/r)*(h.tier===2?1:h.tier===1?.75:.5); if(g>max) max=g }
  }
  return max
}
...
// Hit-test a canvas click against hotspot projected positions
function findClickedHotspot(
  mx: number, my: number,
  rotLon: number, rotTheta: number, scale: number
): Hotspot | null {
  const CX = SIZE/2, CY = SIZE/2
  const R = GLOB_R * scale
  let best: Hotspot | null = null
  let bestD = 999
  for (const h of HOTSPOTS) {
    const dLon = ((h.lon - rotLon + 540) % 360) - 180
    if (Math.abs(dLon) >= 90) continue
    const la = h.lat*Math.PI/180, lo = dLon*Math.PI/180
    const x = Math.cos(la)*Math.sin(lo)
    const y = Math.sin(la), z = Math.cos(la)*Math.cos(lo)
    const y2 = y*Math.cos(rotTheta)+z*Math.sin(rotTheta)
    const z2 = -y*Math.sin(rotTheta)+z*Math.cos(rotTheta)
    if (z2 < 0.01) continue
    const sx = CX+x*R, sy = CY-y2*R
    const d = Math.hypot(mx-sx, my-sy)
    const hitR = (h.tier===2?18:h.tier===1?14:10) * scale
...
export function NektaGlobe() {
  ...
  const flyTo = useCallback((h: Hotspot) => {
    ...
    setSpinning(false)           // stop spinning when a city is selected
    setActiveHotspot(h)
  ...
  const dismissHotspot = useCallback(() => {
    ...
    setSpinning(true)            // resume spin on dismiss
  }, [])
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Escape') dismissHotspot()
    }
  ...
  useEffect(() => {
    ...
    function draw() {
      // Only auto-spin if spinning is enabled and not dragging
      if (spinningRef.current && !pointerDownRef.current) {
        targetLonRef.current += 0.1
      }
      ...
      for (const dot of dotsRef.current) {
        ...
        if (!p) continue
        const alpha = Math.min(1, 0.5+p.z*0.45)
      ...
      for (const h of HOTSPOTS) {
        ...
        if (!p) continue
        const isActive = activeHotspot?.name === h.name  // captured in closure — will always be stale; we use canvas hit-test instead
        const pulse = Math.sin(pulseRef.current + h.lat) * 0.5 + 0.5
        const ts = h.tier===2?1:h.tier===1?.75:.55

        // Pulse ring
        const ringR = (6+pulse*8)*ts*scaleRef.current*p.z
        ctx.beginPath(); ctx.arc(p.x, p.y, ringR, 0, Math.PI*2)
        ctx.strokeStyle = `rgba(255,140,30,${((0.3-pulse*0.25)*p.z).toFixed(2)})`
        ctx.lineWidth = 0.8; ctx.stroke()

        // Glow
        const glowR = (4+pulse*3)*ts*scaleRef.current*p.z
        const grd = ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,glowR)
        grd.addColorStop(0,`rgba(255,200,80,${(0.9*p.z).toFixed(2)})`)
        grd.addColorStop(0.4,`rgba(255,120,20,${(0.5*p.z).toFixed(2)})`)
        grd.addColorStop(1,'rgba(255,65,10,0)')
        ctx.beginPath(); ctx.arc(p.x,p.y,glowR,0,Math.PI*2); ctx.fillStyle=grd; ctx.fill()

        // Core
        const coreR = (1.8+pulse*0.6)*ts*scaleRef.current*p.z
        ctx.beginPath(); ctx.arc(p.x,p.y,coreR,0,Math.PI*2)
        ctx.fillStyle=`rgba(255,240,180,${Math.min(1,p.z*1.1).toFixed(2)})`
        ctx.fill()
      }
  ...
  // Canvas click — hit test hotspot dots directly
  const onCanvasClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (hit) {
      ...
      // Click on empty space — dismiss and resume spin
      dismissHotspot()
    }
  }, [flyTo, dismissHotspot])
  ...
  const onTouchEnd = useCallback((e: React.TouchEvent) => {
    if (!didDragRef.current && e.changedTouches.length===1) {
      ...
      const hit = findClickedHotspot(mx,my,rotLonRef.current,rotThetaRef.current,scaleRef.current)
      if (hit) flyTo(hit); else dismissHotspot()
    }
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={S.spinStatus} onClick={() => setSpinning(s => !s)}>
        {spinning ? '⏸ SPACE TO PAUSE' : '▶ SPACE TO SPIN'}
      </div>
      ...
      <div style={S.sidebar}>
        {HOTSPOTS.map(h=>(
          <button key={h.name} onClick={()=>flyTo(h)}
            style={{...S.si, opacity:activeHotspot?.name===h.name?1:0.45, background:activeHotspot?.name===h.name?'rgba(255,65,32,0.08)':'none'}}>
            <span style={{...S.dot,...(h.tier===2?S.dotBig:h.tier===1?S.dotMid:{})}}/>
            <span style={S.label}>{h.name}</span>
            {events[h.name]&&<span style={S.badge}>{events[h.name].length}</span>}
          </button>
        ))}
      </div>
      ...
      {activeHotspot&&(
        <div style={S.panel}>
          <div style={S.panelHeader}>
            ...
            <button onClick={dismissHotspot} style={S.closeBtn}>✕</button>
          </div>
          {cityEvents.length>0 ? (
              ...
              {cityEvents.map(ev=>(
                <div key={ev.id} style={S.eventRow}>
                  ...
                  <div style={S.eventMeta}>
                    {ev.venue?.name} · {new Date(ev.startTime).toLocaleDateString('en-GB',{weekday:'short',day:'numeric',month:'short'})}
                  </div>
      ...
      <div style={S.hint}>click a dot · drag to rotate · ctrl+scroll to zoom</div>
    </div>
  )
}
...
const S: Record<string,React.CSSProperties> = {
  ...
  spinStatus:{position:'absolute',top:'24px',right:'20px',fontSize:'8px',color:'rgba(255,65,32,0.35)',letterSpacing:'0.12em',cursor:'pointer',zIndex:10,padding:'4px 8px',border:'1px solid rgba(255,65,32,0.15)',borderRadius:'4px'},
  ...
  sidebar:{position:'absolute',left:'20px',top:'50%',transform:'translateY(-50%)',display:'flex',flexDirection:'column',gap:'4px',maxHeight:'80vh',overflowY:'auto',zIndex:10},
  si:{display:'flex',alignItems:'center',gap:'7px',cursor:'pointer',padding:'4px 8px',borderRadius:'4px',border:'none',transition:'all 0.2s'},
